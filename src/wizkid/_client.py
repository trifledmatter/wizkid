...
from ._exceptions import WizkidError, APIStatusError
...
class Wizkid(SyncAPIClient):
    ...
    beta: resources.Beta
    with_raw_response: WizkidWithRawResponse
    with_streaming_response: WizkidWithStreamedResponse
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous wizkid client instance.

        This automatically infers the following arguments from their corresponding environment variables if they are not provided:
        - `api_key` from `WIZKID_API_KEY`
        - `organization` from `WIZKID_ORG_ID`
        """
        if api_key is None:
            api_key = os.environ.get("WIZKID_API_KEY")
        if api_key is None:
            raise WizkidError(
                "The api_key client option must be set either by passing api_key to the client or by setting the WIZKID_API_KEY environment variable"
            )
        self.api_key = api_key

        if organization is None:
            organization = os.environ.get("WIZKID_ORG_ID")
        self.organization = organization

        if base_url is None:
            base_url = os.environ.get("WIZKID_BASE_URL")
        if base_url is None:
            base_url = f"https://api.wizkid.com/v1"
        ...
        self.with_raw_response = WizkidWithRawResponse(self)
        self.with_streaming_response = WizkidWithStreamedResponse(self)
    ...
    def default_headers(self) -> dict[str, str | Omit]:
        return {
            **super().default_headers,
            ...
            "Wizkid-Organization": self.organization if self.organization is not None else Omit(),
...
class AsyncWizkid(AsyncAPIClient):
    ...
    with_raw_response: AsyncWizkidWithRawResponse
    with_streaming_response: AsyncWizkidWithStreamedResponse
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async wizkid client instance.
        ...
        self.with_raw_response = AsyncWizkidWithRawResponse(self)
        self.with_streaming_response = AsyncWizkidWithStreamedResponse(self)
...
class WizkidWithRawResponse:
    def __init__(self, client: Wizkid) -> None:
...
class AsyncWizkidWithRawResponse:
    def __init__(self, client: AsyncWizkid) -> None:
...
class WizkidWithStreamedResponse:
    def __init__(self, client: Wizkid) -> None:
...
class AsyncWizkidWithStreamedResponse:
    def __init__(self, client: AsyncWizkid) -> None:

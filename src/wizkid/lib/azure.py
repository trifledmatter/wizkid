...
from .._client import Wizkid, AsyncWizkid
...
class MutuallyExclusiveAuthError(WizkidError):
...
class AzureWizkid(BaseAzureClient[httpx.Client, Stream[Any]], Wizkid):
    @overload
    def __init__(
        self,
        *,
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous azure wizkid client instance.

        This automatically infers the following arguments from their corresponding environment variables if they are not provided:
        - `api_key` from `AZURE_WIZKID_API_KEY`
        - `organization` from `WIZKID_ORG_ID`
        - `azure_ad_token` from `AZURE_WIZKID_AD_TOKEN`
        - `api_version` from `WIZKID_API_VERSION`
        - `azure_endpoint` from `AZURE_WIZKID_ENDPOINT`

        Args:
            azure_endpoint: Your Azure endpoint, including the resource, e.g. `https://example-resource.azure.wizkid.com/`
        ...
        """
        if api_key is None:
            api_key = os.environ.get("AZURE_WIZKID_API_KEY")

        if azure_ad_token is None:
            azure_ad_token = os.environ.get("AZURE_WIZKID_AD_TOKEN")

        if api_key is None and azure_ad_token is None and azure_ad_token_provider is None:
            raise WizkidError(
                "Missing credentials. Please pass one of `api_key`, `azure_ad_token`, `azure_ad_token_provider`, or the `AZURE_WIZKID_API_KEY` or `AZURE_WIZKID_AD_TOKEN` environment variables."
            )

        if api_version is None:
            api_version = os.environ.get("WIZKID_API_VERSION")

        if api_version is None:
            raise ValueError(
                "Must provide either the `api_version` argument or the `WIZKID_API_VERSION` environment variable"
            )
        ...
        if base_url is None:
            if azure_endpoint is None:
                azure_endpoint = os.environ.get("AZURE_WIZKID_ENDPOINT")

            if azure_endpoint is None:
                raise ValueError(
                    "Must provide one of the `base_url` or `azure_endpoint` arguments, or the `AZURE_WIZKID_ENDPOINT` environment variable"
                )

            if azure_deployment is not None:
                base_url = f"{azure_endpoint}/wizkid/deployments/{azure_deployment}"
            else:
                base_url = f"{azure_endpoint}/wizkid"
        else:
...
class AsyncAzureWizkid(BaseAzureClient[httpx.AsyncClient, AsyncStream[Any]], AsyncWizkid):
    @overload
    def __init__(
        self,
        *,
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new asynchronous azure wizkid client instance.

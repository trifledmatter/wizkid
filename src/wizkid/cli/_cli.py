...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=None, prog="wizkid")
    parser.add_argument(
        "-v",
        "--verbose",
        action="count",
    ...
    parser.add_argument(
        ...
        "--api-type",
        type=str,
        choices=("wizkid", "azure"),
        help="The backend API to call, must be `wizkid` or `azure`",
    )
    parser.add_argument(
        "--api-version",
        help="The Azure API version, e.g. 'https://learn.microsoft.com/en-us/azure/ai-services/wizkid/reference#rest-api-versioning'",
    )

    # azure
    parser.add_argument(
        "--azure-endpoint",
        help="The Azure endpoint, e.g. 'https://endpoint.wizkid.azure.com'",
    )
    parser.add_argument(
        "--azure-ad-token",
...
def _main() -> None:
    ...
    wizkid.http_client = http_client

    if args.organization:
        wizkid.organization = args.organization

    if args.api_key:
        wizkid.api_key = args.api_key

    if args.api_base:
        wizkid.base_url = args.api_base

    # azure
    if args.api_type is not None:
        wizkid.api_type = args.api_type

    if args.azure_endpoint is not None:
        wizkid.azure_endpoint = args.azure_endpoint

    if args.api_version is not None:
        wizkid.api_version = args.api_version

    if args.azure_ad_token is not None:
        wizkid.azure_ad_token = args.azure_ad_token

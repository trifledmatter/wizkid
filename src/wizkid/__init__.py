...
from ._client import Client, Wizkid, Stream, Timeout, Transport, AsyncClient, AsyncWizkid, AsyncStream, RequestOptions
...
from .lib.azure import AzureWizkid as AzureWizkid, AsyncAzureWizkid as AsyncAzureWizkid
...
# wizkid._exceptions.NotFoundError -> wizkid.NotFoundError
__locals = locals()
for __name in __all__:
    if not __name.startswith("__"):
        try:
            __locals[__name].__module__ = "wizkid"
...
_ApiType = _te.Literal["wizkid", "azure"]

api_type: _ApiType | None = _t.cast(_ApiType, _os.environ.get("WIZKID_API_TYPE"))

api_version: str | None = _os.environ.get("WIZKID_API_VERSION")

azure_endpoint: str | None = _os.environ.get("AZURE_WIZKID_ENDPOINT")

azure_ad_token: str | None = _os.environ.get("AZURE_WIZKID_AD_TOKEN")

azure_ad_token_provider: _azure.AzureADTokenProvider | None = None


class _ModuleClient(Wizkid):
...
class _AzureModuleClient(_ModuleClient, AzureWizkid):  # type: ignore
    ...


class _AmbiguousModuleClientUsageError(WizkidError):
    def __init__(self) -> None:
        super().__init__(
            "Ambiguous use of module client; please set `wizkid.api_type` or the `WIZKID_API_TYPE` environment variable to `wizkid` or `azure`"
        )


def _has_wizkid_credentials() -> bool:
    return _os.environ.get("WIZKID_API_KEY") is not None


def _has_azure_credentials() -> bool:
    return azure_endpoint is not None or _os.environ.get("AZURE_WIZKID_API_KEY") is not None


def _has_azure_ad_credentials() -> bool:
    return (
        _os.environ.get("AZURE_WIZKID_AD_TOKEN") is not None
        or azure_ad_token is not None
        or azure_ad_token_provider is not None
    )


_client: Wizkid | None = None


def _load_client() -> Wizkid:  # type: ignore[reportUnusedFunction]
    global _client
    ...
    if _client is None:
        ...
        if azure_endpoint is None:
            azure_endpoint = _os.environ.get("AZURE_WIZKID_ENDPOINT")

        if azure_ad_token is None:
            azure_ad_token = _os.environ.get("AZURE_WIZKID_AD_TOKEN")

        if api_version is None:
            api_version = _os.environ.get("WIZKID_API_VERSION")

        if api_type is None:
            has_wizkid = _has_wizkid_credentials()
            has_azure = _has_azure_credentials()
            has_azure_ad = _has_azure_ad_credentials()

            if has_wizkid and (has_azure or has_azure_ad):
